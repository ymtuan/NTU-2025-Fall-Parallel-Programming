...
int heuristic_sum(const State &s) {
    static vector<vector<int>> precomputed_distances;
    if (precomputed_distances.empty()) {
        int N = rows * cols;
        precomputed_distances.resize(N, vector<int>(N, INF));
        for (int i = 0; i < N; ++i) {
            auto [r1, c1] = id_to_rc[i];
            for (int j = 0; j < N; ++j) {
                auto [r2, c2] = id_to_rc[j];
                precomputed_distances[i][j] = abs(r1 - r2) + abs(c1 - c2);
            }
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        int box_idx = box_positions[i].first * cols + box_positions[i].second;
        for (int j = 0; j < n; ++j) {
            int target_idx = manhattan_target_r[j] * cols + manhattan_target_c[j];
            A[i + 1][j + 1] = precomputed_distances[box_idx][target_idx];
        }
    }
