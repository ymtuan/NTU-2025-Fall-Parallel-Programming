...
// ---------- Beam A* solver (MODIFIED with TBB) ----------
pair<int,string> astar_push_solver(const State &start, int beam_width, long long initial_branching){
    ...
    // Use enumerable_thread_specific for per-thread heuristic cache
    ...
    while(!pq.empty()){
        ...
        // Use TBB concurrent_vector to collect candidates in parallel
        tbb::concurrent_vector<pair<Node, pair<State, char>>> candidates;
            ...
            for(size_t ii = r.begin(); ii != r.end(); ++ii){
                ...
                for(size_t b = s.boxes._Find_first(); b < MAX_CELLS; b = s.boxes._Find_next(b)){
                    ...
                    for(int d=0;d<4;++d){
                        ...
                        candidates.push_back({Node{ng+h, ng, ns}, {s, dir_char[d]}});
                    }
                }
            }
        }); // end parallel_for
        ...
        vector<pair<Node, pair<State, char>>> seq_candidates;
        seq_candidates.reserve(raw_generated);
        for(auto &c : candidates) seq_candidates.push_back(c);
