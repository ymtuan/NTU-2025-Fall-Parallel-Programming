...
__constant__ double d_sin_table_const[4096];
...
__global__ void __launch_bounds__(BLOCK_SIZE) step_kernel_one_block_per_body(
    ...
    #if USE_CONST_SIN_TABLE
    double sin_val = d_sin_table_const[step];
    #else
...
// Single-block kernel for small N (N <= 256)
// Runs multiple timesteps in one kernel launch
...
// Batch kernel for P3: test device destruction scenarios from checkpoints
...
public:
    Simulator(int dev_id, const SimParams& p, const std::vector<double>& h_m, const std::vector<int>& h_t, const std::vector<double>& h_sin) 
        ...
        #if USE_CONST_SIN_TABLE
        if (p.n_steps + 1 <= 4096) {
            HIP_CHECK(hipMemcpyToSymbol(d_sin_table_const, h_sin.data(), 
                                       (p.n_steps + 1) * sizeof(double)));
        }
        #endif
...
int main(int argc, char** argv) {
    ...
    Simulator sim0(0, params, m, type, sin_table); 
    Simulator sim1(1, params, m, type, sin_table);
